...
# Where to look for the panel at all: the right fifth of the frame. Generous on purpose — it
# BRACKETS the search, and the panel's real edges are measured from what is found inside it.
# A region is a safe use of a rough constant; a tap target is not.
_SEARCH_X_MIN_NORM = 0.72

# A tab is roughly square and small. The list rows below are much wider than they are tall,
# and the minimap is much bigger, so this separates the strip from both without a y constant.
_TAB_MAX_ASPECT = 2.0
_TAB_MIN_SIDE = 40
_TAB_MAX_SIDE = 160

# Rows of the list are at least this tall. Below it are the sub-labels that belong to the row
# above (a destination's "Approx. 65.7km"), which must not be counted as rows of their own.
_MIN_ROW_H = 45

# How far a part may poke outside the measured span and still belong to the panel. Small,
# because the thing it must exclude — the sea's gauge column — is immediately adjacent.
_EDGE_SLACK = 12
...
def detect_overworld_panel(frame, elements: Sequence[Any] = None) -> Optional[OverworldPanel]:
    ...
    width = _width(frame, els)
    right = [e for e in els if _cx(e) >= width * _SEARCH_X_MIN_NORM]
    if not right:
        return None

    season = _season_row(right)
    if season is None:
        # NO SEASON ROW, NO PANEL. Deliberately strict: without the anchor every other part
        # would have to be found by a y constant, which is the thing this module exists to
        # stop. A caller that wants "is there anything on the right" should ask the chrome
        # detector, which is what that count is for.
        logger.debug("[overworld_panel] no season row on the right — not this screen")
        return None

    x0, x1 = _span(right, season)
    tabs = _tabs(right, x0, x1, above=season[1])
    ...
    rows = _rows(right, x0, x1, below=(season[1] + season[3]) // 2)
    ...
    bottom = min(_height(frame, els), max((r[2] for r in rows), default=0) + _MIN_ROW_H)
    list_region = (x0, season[3], x1, bottom) if rows else None

    return OverworldPanel(box=(x0, top, x1, list_region[3] if list_region else season[3]),
                          tabs=tuple(tabs), minimap=minimap, season_row=season,
                          list_region=list_region, rows=tuple(rows))
...
def _span(els, season) -> Tuple[int, int]:
    ...
    """
    wide = [e for e in els if (e.x2 - e.x1) > (season[2] - season[0]) * 0.8]
    if not wide:
...
def _tabs(els, x0: int, x1: int, *, above: int) -> List[Tab]:
    ...
    cands = [e for e in els
             if e.y2 < above
             and e.x1 >= x0 - _EDGE_SLACK and e.x2 <= x1 + _EDGE_SLACK
             and _MIN_SIDE_OK(e)]
    if not cands:
        return []
...
def _MIN_SIDE_OK(e) -> bool:
    w, h = e.x2 - e.x1, e.y2 - e.y1
    if not (_TAB_MIN_SIDE <= h <= _TAB_MAX_SIDE):
        return False
    return h > 0 and (w / h) <= _TAB_MAX_ASPECT
...
def _rows(els, x0: int, x1: int, *, below: int) -> List[Tuple[str, int, int]]:
    """The list, as (label, x, y) — the shape `read_building_menu` already returns.

    A row is a LABELLED box below the season row, wide enough to span the panel. The sub-label
    under a destination ("Approx. 65.7km") is narrower and shorter, and belongs to the row
    above rather than being one.
    """
    out = []
    for e in sorted((e for e in els if e.y1 >= below), key=lambda e: e.y1):
        label = (getattr(e, "label", "") or "").strip()
        if not label or (e.y2 - e.y1) < _MIN_ROW_H:
            continue
        if (e.x2 - e.x1) < (x1 - x0) * 0.6:
            continue
