...
# ── The shared tab-strip reading ──────────────────────────────────────────────
#
# THESE STAYED when the port panel moved to `actions/port_panel.py` (2026-09-11).
# `selected_tab_index` below answers "which tab is lit" for the PORT strip and for
# the WORLD MAP strip, which is a different element with a different vocabulary and
# no trailing toggle. The reading is the same luminance test either way, so it has
# one home that both can reach.
